...
Everything under [`../data`](../data) is **raw** experimental output — the
numbers behind the figures and tables in the paper. Rendered graphs are
deliberately *not* committed; the plotters in [`../scripts`](../scripts)
regenerate them from these files (see the top-level
[README](../README.md)).
...
## `campaign_felix/` — overhead study (paper §4.1)
...
full timing distribution per row; the paper reports the **mean**.
...
## `comparison_felix/` and `comparison_pacca/` — head-to-head (paper §4.2)
